...
const getRooms = catchAsyncErrors(async (req, res) => {
  const resPerPage = 4;
...
const getRoom = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!room) {
    return next(new ErrorHandler("Room not found with provided ID", 404));
  }
  res.status(200).json({
    success: true,
    room,
...
export { getRooms, createRoom, getRoom, updateRoom, deleteRoom };
